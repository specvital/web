---
title: Collector 중심 분석 라이프사이클
description: 분석 레코드 라이프사이클 소유권을 Collector 서비스에 위임하는 ADR
---

# ADR-12: Collector 중심 분석 라이프사이클

> 🇺🇸 [English Version](/en/adr/12-collector-centric-analysis-lifecycle.md)
...
ADR-03에서 API(Web)와 Worker(Collector) 서비스 분리 확립. ADR-04에서 큐 기반 비동기 처리 도입. 초기 구현의 이중 소유권 문제 발생:

**기존 흐름:**

```
사용자 요청 → Web이 "pending" 레코드 생성 → Enqueue → Collector 처리 → 레코드 업데이트
```
...
| 문제             | 영향                                                         |
| ---------------- | ------------------------------------------------------------ |
| 중복 레코드      | Web이 pending 레코드 생성, Collector도 별도 레코드 생성 가능 |
...
핵심 문제: 분석 레코드 라이프사이클에 대한 **단일 진실 공급원(Single Source of Truth) 부재**. Web과 Collector 모두 동일 레코드에 쓰기 권한 보유로 동기화 복잡성 야기.

## 결정

**Collector가 레코드 생성, 처리, 완료를 독점적으로 소유하는 Collector 중심 분석 라이프사이클 채택.**

### 새로운 아키텍처

```
사용자 요청 → Web (enqueue만) → Queue → Collector (생성 → 처리 → 완료)
                    ↓                              ↓
...
2. **Collector: 전체 소유** - 작업 시작 시 레코드 생성, 완료 시 업데이트
3. **Queue가 상태 소스** - Web은 진행 중 분석을 DB가 아닌 큐에서 확인
4. **단일 Writer** - Collector만 분석 레코드에 쓰기

## 검토한 옵션

### 옵션 A: Collector 중심 소유권 (선택)

Web이 생성된 UUID와 함께 분석 요청 enqueue. Collector가 처리 시작 시 레코드 생성, 완료 시 갱신.

**장점:**

- 단일 진실 공급원: 하나의 서비스가 전체 라이프사이클 소유
- 중복 레코드 없음: Collector만 분석 항목 생성
- 명확한 서비스 경계: Web은 HTTP, Collector는 분석 담당
- 단순한 에러 처리: 모든 실패 상태를 한 곳에서 관리
- 트랜잭션 일관성 향상: 동일 서비스 컨텍스트에서 생성과 업데이트
- 독립적 확장: Collector가 Web에 영향 없이 확장 가능
...
Web이 모든 레코드 생성 및 관리. Collector는 기존 레코드만 업데이트.
...
- Collector가 큐 깊이에 따라 독립적으로 확장
- Web은 가벼움 유지 (분석용 무거운 DB 쓰기 없음)
- 큐가 자연스럽게 부하 버퍼링

**미래 호환성:**

- 예약 재수집과 정렬 (Collector ADR-01)
...
- Collector가 처리 시작할 때까지 분석이 DB에 미노출
...
| 측면          | 함의                                                  |
| ------------- | ----------------------------------------------------- |
| 트랜잭션 범위 | 레코드 생성 + 초기 상태가 단일 Collector 트랜잭션에서 |
| 실패 처리     | 모든 재시도가 Collector에서 관리                      |
...
- [Collector ADR-01: 스케줄 기반 재수집 아키텍처](/ko/adr/collector/01-scheduled-recollection.md)
